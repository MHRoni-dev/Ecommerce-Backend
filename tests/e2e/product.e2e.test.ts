...
} from '@e2e/__fixtures__/productData';
...
describe('Product E2E test', () => {
  async function createProduct(data: object) {
    return await request(app).post('/api/v1/product/create').send(data);
  }

  async function updateProduct(slug: string, data: object) {
    return await request(app).put(`/api/v1/product/update/${slug}`).send(data);
  }

  async function readProduct(slug: string) {
    return await request(app).get(`/api/v1/product/read/${slug}`);
  }

  async function deleteProduct(slug: string) {
    return await request(app).delete(`/api/v1/product/delete/${slug}`);
  }
  ...
  */
  ...
  it('should read all createdProduct', async () => {
    ...
    const res = await request(app).get('/api/v1/product/read');
  ...
  it('should read currentProduct with currentSlug', async () => {
    ...
    const createdProduct: Product = createRes.body.product;
  ...
  it('should read product with oldSlug', async () => {
    ...
    let currentProduct: Product = createRes.body.product;
    ...
    for (let i = 0; i <= 5; i++) {
      ...
      expect(res.body.product).toHaveProperty(
        'slug',
        expect.not.stringMatching(currentSlug),
      );
  ...
  it('should update product', async () => {
    ...
    */
    ...
    expect(res.body.product).toHaveProperty(
      'title',
      validUpdateProductData.title,
    );
    expect(res.body.product).toHaveProperty(
      'slug',
      expect.not.stringMatching(createdProduct.slug),
    );
    expect(res.body.product).toHaveProperty(
      'price',
      validUpdateProductData.price,
    );
  });
  ...
  it('should not let update with the oldSlug', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const updateRes = await updateProduct(currentSlug, {
        title: `update no ${i + 1}`,
      });
  ...
  it('should delete product with currentSlug', async () => {
    ...
    const product: Product = createRes.body.product;
    //delete phase
  ...
  it('should not delete with oldSlug', async () => {
    ...
    for (let slug of slugList) {
      const res = await request(app).delete(`/api/v1/product/delete/${slug}`);
  ...
  it('should not found any ProductRedirect of the deleted product', async () => {
    ...
    const res = await mongoose
      ...
      .findOne({ productId: deleteRes.body.product._id });
